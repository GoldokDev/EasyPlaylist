...
## D-031 — Blind test expurgé avec exception du navigateur lecteur

- Date : 2026-08-09
- Statut : `ACCEPTED`
- Décision : permettre au créateur d'activer à tout moment un mode blind test persistant. Le serveur remplace alors la file publique par son seul compteur, suspend retrait et réordonnancement, présente le titre courant comme « Musique de {pseudonyme} » et supprime le titre des transitions. Recherche, ajout, bail et commandes de lecture restent ouverts à tous. Seul le détenteur du bail reçoit `provider` et `providerTrackId` pour exécuter la lecture ; aucun reveal, minuteur, réponse ou score n'est inclus.
- Pourquoi : masquer seulement l'interface laisserait titres, miniatures et identifiants fournisseur accessibles dans HTTP et Socket.IO. Des unions Zod distinctes rendent ces champs impossibles à sérialiser dans les snapshots blind test. La bascule réutilise la version et le verrou du lobby afin qu'un ancien snapshot détaillé ne remplace pas le mode déjà observé.
- Limite acceptée : l'IFrame YouTube reste visible, mesure au moins 200 × 200 et n'est jamais recouverte sur le navigateur lecteur, conformément aux [fonctionnalités minimales YouTube](https://developers.google.com/youtube/terms/required-minimum-functionality) vérifiées le 2026-08-09. Son détenteur peut donc voir la réponse ; les autres appareils ne créent pas de lecteur et ne reçoivent pas l'identifiant vidéo. Une activation tardive ne peut pas effacer les titres déjà vus et l'interface le rappelle avant confirmation.
- Conséquence : `PATCH /lobbies/:id/settings` et `lobby.settings.updated` rejoignent le contrat public. `QueueSnapshot` et `PlayerSnapshot` deviennent des unions discriminées par `blindTestEnabled`; le second porte aussi `lobbyVersion`. D-024 reste applicable au snapshot normal mais son contenu complet est remplacé par un compteur en blind test. L'exception de droits de D-006/Q-006 suspend seulement retrait et réordonnancement pendant ce mode, sans introduire de rôle de modération.
